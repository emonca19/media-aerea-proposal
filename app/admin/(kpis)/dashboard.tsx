...
export default function AdminKPIDashboard() {
  ...
  const loadKPIData = async () => {
    try {
      ...
      const totalTurbines = projectData.reduce((sum, p) => sum + p.progress.totalTurbines, 0);
      const completedTurbines = projectData.reduce((sum, p) => sum + p.progress.completed, 0);
      const totalFlightHours = pilotData.reduce((sum, p) => sum + p.totalFlightMinutes, 0) / 60;
      const averagePilotEfficiency = pilotData.reduce((sum, p) => sum + p.dailyCompletionRatePercentage, 0) / pilotData.length;
      ...
      setKpiData({
        projects: {
          ...
          active: 8,
          completed: 4,
          averageProgress: projectData.reduce((sum, p) => sum + p.progress.completionPercentage, 0) / projectData.length,
          delayedProjects: 2
        },
        clients: {
          total: 5,
        ...
        turbines: {
          ...
          approvalRate: projectData.reduce((sum, p) => sum + p.qualityMetrics.approvalRate, 0) / projectData.length
        },
        pilots: {
    ...
    <View style={styles.container}>
      <Stack.Screen 
        options={{
          ...
          headerRight: () => (
            <TouchableOpacity onPress={loadKPIData} style={styles.refreshButton}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View entering={FadeInUp.delay(100)} style={styles.headerCard}>
          ...
          >
            <View style={styles.headerContent}>
              ...
              <Text style={styles.headerSubtitle}>Estado actual del sistema</Text>
              <View style={styles.headerStats}>
                <View style={styles.headerStat}>
                  <Text style={styles.headerStatValue}>{kpiData.projects.active}</Text>
                  <Text style={styles.headerStatLabel}>Proyectos Activos</Text>
                </View>
                <View style={styles.headerStat}>
                  <Text style={styles.headerStatValue}>{kpiData.pilots.active}</Text>
                  <Text style={styles.headerStatLabel}>Pilotos Activos</Text>
                </View>
                <View style={styles.headerStat}>
                  <Text style={styles.headerStatValue}>{Math.round(kpiData.pilots.averageEfficiency)}%</Text>
          ...
          <View style={styles.turbineProgress}>
            ...
            <View style={styles.progressBar}>
              <View 
                style={[
                  styles.progressFill, 
                  { 
                    width: `${(kpiData.turbines.inspected / kpiData.turbines.total) * 100}%`,
            ...
            <View style={styles.progressLabels}>
              <Text style={styles.progressLabel}>
                Inspeccionadas: {kpiData.turbines.inspected}/{kpiData.turbines.total}
              </Text>
              <Text style={styles.progressPercentage}>
                {Math.round((kpiData.turbines.inspected / kpiData.turbines.total) * 100)}%
              </Text>
            </View>
          </View>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.section}>
          <Text style={styles.sectionTitle}>Estadísticas Individuales de Pilotos</Text>
              ...
              <View style={styles.pilotHeader}>
                <View style={styles.pilotInfo}>
                  ...
                  <Text style={styles.pilotStats}>
                    {pilot.totalTurbinesInspected} turbinas • {Math.round(pilot.totalFlightMinutes / 60)}h vuelo
                  </Text>
                </View>
              ...
              <View style={styles.pilotDetailsGrid}>
                <View style={styles.pilotDetail}>
                  ...
                  <Text style={styles.pilotDetailText}>
                    {Math.round(pilot.averageTimePerTurbineSeconds / 3600)}h por turbina
                  </Text>
                </View>
                <View style={styles.pilotDetail}>
              ...
              <View style={styles.bottleneckHeader}>
                <View style={styles.bottleneckInfo}>
                  <View style={styles.bottleneckTitleRow}>
                    ...
                    />
                    <Text style={styles.bottleneckCategory}>{bottleneck.category}</Text>
                  </View>
                  <Text style={styles.bottleneckDescription}>{bottleneck.description}</Text>
                </View>
                <View style={styles.bottleneckMetrics}>
                  <Text style={[styles.bottleneckCount, { color: getImpactColor(bottleneck.impact) }]}>
                    {bottleneck.count}
                  </Text>
                  <Text style={styles.bottleneckCountLabel}>casos</Text>
                </View>
              </View>
              <View style={[styles.impactBadge, { backgroundColor: getImpactColor(bottleneck.impact) + '20' }]}>
                <Text style={[styles.impactText, { color: getImpactColor(bottleneck.impact) }]}>
                  Impacto {bottleneck.impact === 'high' ? 'Alto' : bottleneck.impact === 'medium' ? 'Medio' : 'Bajo'}
                </Text>
              </View>
            </View>
          ))}
